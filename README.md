...
s3prefix-archive is built around **list → filter → get → encode**. AWS IAM is usually split between **`s3:ListBucket`** (often constrained with `Condition` `StringLike` on `s3:prefix`) and **`s3:GetObject`** on specific keys or prefixes. Your integration should match how IAM policies and roles expose those actions.

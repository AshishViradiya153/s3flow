...
21. [Developing s3flow](#developing-s3flow)
...
2. Ensure AWS credentials are available to the SDK (environment, shared config, IAM role, etc.). Pass an **`S3Client`** into s3flow options in production (see [Configuration and credentials](#configuration-and-credentials)).
...
**Optional peers: GCS / Azure (`s3flow/gcs`, `s3flow/azure-blob`)**
...
| `s3flow/gcs`        | `GcsStorageProvider` for **Google Cloud Storage** lists + reads (install `@google-cloud/storage` peer).        |
| `s3flow/azure-blob` | `AzureBlobStorageProvider` for **Azure Blob Storage** (install `@azure/storage-blob` peer).                    |
...
**Reuse the index between runs:** keep `prefix.ndjson` on disk or in object storage, then pass `fs.createReadStream("prefix.ndjson")` (or another `Readable`) as **`preparedIndexNdjson`** on later archives so you avoid a second **`ListObjectsV2`** until you refresh the file. Expiry / invalidation is operator-owned—s3flow does not cache or TTL the NDJSON for you.

### Selective files, multiple folders, and IAM

s3flow is built around **list → filter → get → encode**. AWS IAM is usually split between **`s3:ListBucket`** (often constrained with `Condition` `StringLike` on `s3:prefix`) and **`s3:GetObject`** on specific keys or prefixes. Your integration should match how your org grants those actions.

| Goal                                                                       | What s3flow does                                                                                                                                                                                                                    | IAM / operations note                                                                                                                                                                                                                            |
| -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |

...
- **First run with checkpoint:** pass **`checkpoint: { jobId, store }`** to **`downloadFolderToFile`** / **`pumpArchiveToWritable`** (`s3flow`) or **`runFolderArchiveToS3`** (`s3flow/platform`) — same semantics as a run without resume, but state is persisted.

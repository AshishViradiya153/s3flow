...
- **First run with checkpoint:** pass **`checkpoint: { jobId, store }`** to **`downloadFolderToFile`** / **`pumpArchiveToWritable`** (`@ashishviradiya153/s3download`) or **`runFolderArchiveToS3`** (`@ashishviradiya153/s3download/platform`) — same semantics as a run without resume, but state is persisted.

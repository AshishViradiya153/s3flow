...
AWS IAM typically separates **`s3:ListBucket`** (often scoped with `Condition` on `s3:prefix`) from **`s3:GetObject`**. s3flow needs to align **how you enumerate keys** with what the role allows.

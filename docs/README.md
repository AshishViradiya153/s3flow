# s3flow documentation

In-depth guides for using **s3flow** as an npm package: streaming S3 (and compatible providers) into ZIP / tar / tar.gz archives with production-oriented hooks.
...
| Map imports (`s3flow`, `s3flow/platform`, …) to capabilities | [Reference: exports & modules](reference-exports.md)                           |
...
| `s3flow` CLI (`archive`, `index`, `benchmark`)               | [Guide: CLI](guides/cli.md)                                                    |
...
| Why s3flow is useful to the ecosystem                        | [Community value](community-value.md)                                          |
